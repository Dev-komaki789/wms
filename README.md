...
前職で WMS の開発・保守を担当していましたが、当時は設計の意図や
システム全体像の理解が及ばず、悔しさを残したまま退職しました。
その後は他業種で働いていましたが、「設計が理解できないまま終わりたくない、
今度こそ自分で形にしたい」という気持ちから、職業訓練校ではフロントエンド、
独学では Django を中心としたバックエンドを学び、オリジナルアプリ開発を始めました。
...
### 発注アラート（詳細）
発生時のスナップショット（在庫数・発注点・推奨発注数）を保持し、その後に発注設定を変更しても発生当時の値が残る。「入荷指示を作る」ボタンで 1 クリック変換、入荷完了で自動解決。
