...
urlpatterns = [
    ...
    path(
        'picking-lists/<int:pk>/print/',
        views.PickingListPrintView.as_view(),
        name='picking_list_print',
    ),
    # 出荷明細書印刷（出荷検品完了時に実出荷数で発行された DeliveryNote の帳票印刷）。
    # 箱に同梱して顧客へ届ける書類。
    path(
        'delivery-notes/<int:pk>/print/',
    ...
    path(
        'handheld/picking/<int:pk>/',
        views.OutboundPickingWorkView.as_view(),
        name='handheld_picking_work',
    ),
    # 1 明細ごとの即時コミット API（fetch から呼ばれる）。
    # 全明細が確定したらリストを COMPLETED に進める。
    path(
        'handheld/picking/<int:list_pk>/item/<int:item_pk>/',
        views.OutboundPickingItemView.as_view(),
        name='handheld_picking_item',
    ),
    path(
        'handheld/inspection/', views.OutboundInspectionView.as_view(), name='handheld_inspection'
    ),
    path(
        'handheld/inspection/<int:pk>/',
        views.OutboundInspectionWorkView.as_view(),
        name='handheld_inspection_work',
    ),
    # 1 明細ごとの即時コミット API（fetch から呼ばれる）。
    # 全明細が確定したら ShipmentItem を作成して出荷確定する。
    path(
        'handheld/inspection/<int:order_pk>/item/<int:item_pk>/',
        views.OutboundInspectionItemView.as_view(),
        name='handheld_inspection_item',
    ),
]
