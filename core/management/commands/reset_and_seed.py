...
class Command(BaseCommand):
    ...
    def _create_products_and_skus(self):
        ...
        # leaf カテゴリごとに、そのカテゴリで扱う商品語彙のみを定義する。
        # ここを leaf カテゴリ名と一致させないと、その leaf に紐づく商品が 0 件になる。
        normal_words_by_category = {
            '切削工具': [
        ...
        for category in self._cycle(self.leaf_categories, count=80):
            base, variants = random.choice(normal_words_by_category[category.category_name])
        ...
        for category in self._cycle(self.large_leaf_categories, count=20):
            base, variants = random.choice(large_words_by_category[category.category_name])
            variant = random.choice(variants)
            product = Product.objects.create(
                product_code=f'PRD-{prod_no:05d}',
                product_name=f'{base} {variant}',
